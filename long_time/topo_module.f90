...
module topo_module
    ...
    ! Analytic topography
    real(kind=8), private :: topo_left, topo_right, topo_location(2)
    ...
    real(kind=8), private :: topo_width(2), topo_island, topo_offshore
...
contains
    ...
    subroutine read_topo_settings(file_name)
        ...
        else if (test_topography == 1 .or. test_topography == 4) then
            topo_finalized = .true.
            read(iunit,"(d16.8)") topo_location(1)
        ...
        ! Square Island
        else if (test_topography == 5) then
            topo_finalized = .true.
            read(iunit, *) topo_location
            read(iunit, *) topo_width
            read(iunit, "(d16.8)") topo_island
            read(iunit, "(d16.8)") topo_offshore
        else
...
!         endif
    ...
    real(kind=8) pure function test_topo(x,y) result(topography)
        ...
        real(kind=8) :: radius(2)

        if (test_topography == 1) then
            ! Simple "Riemann" problem like jump
            if (x < topo_location(1)) then
            ...
            ! Island
            radius = 0.5d0 * topo_width
            if (topo_location(1) - radius(1) < x .and.      &
                x < radius(1) + topo_location(1) .and.      &
                topo_location(2) - radius(2) < y .and.      &
                y < radius(2) + topo_location(2)) then

                topography = topo_island
            else
                topography = topo_offshore
            end if
        endif
